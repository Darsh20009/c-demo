...
export default function OSStockManagement() {
  ...
  const createMovementMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: tc("تم بنجاح", "Success"), description: tc("تم تسجيل حركة المخزون", "Inventory movement recorded") });
  ...
  const handleCreateMovement = () => {
    if (!selectedIngredient || !quantity) {
      toast({ title: tc("خطأ", "Error"), description: tc("يرجى ملء جميع الحقول", "Please fill all fields"), variant: "destructive" });
      return;
    }
  ...
  const getMovementLabel = (type: string) => {
    const labels: Record<string, string> = {
      ...
      out: tc("إخراج", "Out"),
      waste: tc("هدر", "Waste"),
      adjustment: tc("تعديل", "Adjustment"),
    };
  ...
  return (
    <div className="p-6 space-y-6 bg-background min-h-screen text-right" dir="rtl">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold tracking-tight">{tc("إدارة حركات المخزون", "Inventory Movements Management")}</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{tc("تسجيل حركة مخزون جديدة", "Record New Inventory Movement")}</CardTitle>
          <CardDescription>{tc("أضف إدخال، إخراج، هدر، أو تعديل للمخزون", "Add inbound, outbound, waste, or adjustment")}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              <Label>{tc("نوع الحركة", "Movement Type")}</Label>
              <Select value={movementType} onValueChange={(v: any) => setMovementType(v)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="in">{tc("إدخال", "In")}</SelectItem>
                  <SelectItem value="out">{tc("إخراج", "Out")}</SelectItem>
                  <SelectItem value="waste">{tc("هدر", "Waste")}</SelectItem>
                  <SelectItem value="adjustment">{tc("تعديل", "Adjustment")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select value={selectedIngredient} onValueChange={setSelectedIngredient}>
                <SelectTrigger>
                  <SelectValue placeholder={tc("اختر", "Select")} />
                </SelectTrigger>
                <SelectContent>
