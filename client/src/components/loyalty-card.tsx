...
export default function LoyaltyCardComponent({ card, showActions = true, compact = false, showTierProgress = true }: LoyaltyCardProps) {
  ...
  useEffect(() => {
    if (qrCanvasRef.current && card.qrToken) {
      ...
      const cardUrl = `https://qiroxstudio.online/loyalty-verify?token=${card.qrToken}`;
      QRCode.toCanvas(
        qrCanvasRef.current,
        cardUrl,
        {
      ...
      {showActions && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button
            ...
            onClick={() => {
              const url = `https://qiroxstudio.online/api/loyalty/cards/${card.id}/apple-wallet-pass`;
